...
%-----------文件输出记录-------------
%记录每个数据集的结果
file_name_result=['..\result_all\',dataname,'_ensembleMVLSD_all_result','.txt'];
file_id_result=fopen(file_name_result,'at+');
%记录每轮结果
file_name_mccv=['..\result_mccv\',dataname,'_each_',vail_type,'_cv','.txt'];
            ...
            for i_view_one=1:(max(mat_sample_num)-1)
                for i_view_two=(i_view_one+1):max(mat_sample_num)
                    ...
                    for i_ensemble=1:sub%采样次数
                        [train_binary_data,train_binary_label]=Sample_Genaration(dataset,vail_type,ktimes,i_classone,i_classtwo,bias_class,para_sampling);
...
[test_data_final,test_label]=TestSample_Genaration(dataset,vail_type,ktimes);
